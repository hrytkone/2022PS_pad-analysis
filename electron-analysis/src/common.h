const double isocut = 5.;
const double isobackthres = 5.;
const bool usefulliso = true;
...
void RemovePedestal()
{
    for (int iasic=0; iasic<nasic; iasic++) {
        for (int ich=0; ich<nch; ich++) {
            //if (ADC[iasic][ich]==1023) ADC[iasic][ich] = 0;
            ADCwoPed[iasic][ich] = (Double_t)ADC[iasic][ich] - Pedestal[iasic][ich];
        }
    }
}

void RemoveCommonNoise(int iasic)
{
    for (int ich=0; ich<nch; ich++)
...
double GetCommonNoiseEdges(int iasic)
{
	vector<double> cmn;
    for (int ich=0; ich<nch; ich++) {
        if (ich==43 || ich==41 || ich==53 || ich==49 || ich==58 || ich==54 || ich==66 ||
            ich==68 || ich==69 || ich==70 || ich==71 || ich==63 || ich==27 || ich==35 ||
            ich==34 || ich==32 || ich==30 || ich==29 || ich==20 || ich==18 || ich==13 ||
            ich==12 || ich==5 || ich==7 || ich==1 || ich==2 || ich==8 || ich==44 ||
            ich==38 || ich==39 )
    	cmn.push_back(ADCwoPed[iasic][ich]);
    }
	std::sort(cmn.begin(), cmn.end());
